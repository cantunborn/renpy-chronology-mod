...
**Cache location and design:** a `TLSnapshotCache` instance lives at `renpy.game.log._tl_snapshot_cache`, with `.menu` (`{node_index: snap}`) and `.chapter` (`{label_name: snap}`) attributes — not subscriptable on the cache object itself. Stored on the log object so it is outside `store` and never in `get_roots()`, eliminating recursive save-file cycles.

The cache is a **reference-sharing / persistent-data-structure** design: it owns exactly one frozen (deep-copied-once) reference for every distinct mutable value ever captured. `_freeze_roots(live_roots)` diffs each live value against the value frozen in the previous capture (`self._last_roots`) via `_tl_values_equal(a, b)` — a single generic comparator (`a is b` fast path, else `pickle.dumps(a) == pickle.dumps(b)`) applied uniformly to every value type, with no assumptions about which game vars do or don't mutate. Unchanged values reuse the prior frozen reference; changed or new values get a fresh `copy.deepcopy`. Because unchanged values across many cached menus end up pointing at the same object, Ren'Py's own single combined save pickle dedupes them for free via its normal memo table when the whole `renpy.game.log` (cache included) is serialized in one `pickle.dump()` call — restoring the memoization the earlier blob/zdict approach had accidentally broken (see changelog).

**Class methods:**

| Method | Signature | Description |
|--------|-----------|-------------|
| `_freeze_roots` | `(self, live_roots) → dict` | The reuse/copy decision described above. `self._last_roots` starts `None`; the first-ever capture has nothing to compare against and deep-copies everything. Updates `self._last_roots` to the newly frozen dict before returning it. |
| `capture` | `(self) → dict` | Flushes pending store deltas (`log.complete(False)`), takes a `rollback_copy()` of the current context, patches `ctx.current` to an enclosing label when needed for compiled `.rpyc` files, records `renpy.game.log.get_roots()` and `renpy.game.log.rollback_limit`, freezes the roots via `self._freeze_roots(...)`. Returns `{"roots": frozen_roots, "ctx": ctx, "rollback_limit": rollback_limit}`. |
| `cache_menu` / `cache_chapter` | `(self, key, snap) → None` | Writes snap to `self.menu[key]` / `self.chapter[key]`. |
...
| Function | Signature | Description |
|----------|-----------|-------------|
| `_tl_build_and_unfreeze` | `(roots, ctx, log_prefix, rollback_limit) → None (never returns)` | Shared tail of both unfreeze paths: forces `ctx.interacting = False`, builds a synthetic single-entry `RollbackLog` with `rollback_limit` set to the value passed in (captured at snapshot time, not hardcoded — `RollbackLog.rollback()` inside `unfreeze()` decrements it by 1 while consuming the entry, the same cost a real load pays, so the player's rollback allowance lands where a real save-then-load at that point would have left it), copies the snapshot cache to the new log, then calls `unfreeze()` — atomically replaces live game state with the captured state. |
| `_tl_unfreeze_legacy` | `(snap) → None (never returns)` | Handles pre-blob `{"roots": ..., "context": ...}` snaps: `copy.deepcopy`s both before use (Ren'Py's real `unfreeze()` aliases roots values directly into `store_dicts`, so without a copy, in-place mutation after the jump would corrupt the cached snapshot). Deepcopy failures propagate rather than falling back to live/aliased references — all-or-nothing, matching the capture-side principle. No historical `rollback_limit` was ever captured for these, so it falls back to `renpy.config.hard_rollback_limit`. Only serves stale, not-yet-revisited nodes — every revisit replaces the cache entry with the current live shape. |
| `_tl_unfreeze_from_snapshot` | `(snap) → None (never returns)` | Dispatches on shape: `"context"` key present → legacy pre-blob shape → `_tl_unfreeze_legacy()`; otherwise the current live shape (`"roots"`/`"ctx"`/`"rollback_limit"`) → deep-copies `roots`/`ctx` fresh (roots may still share frozen references with other cache entries, so the live store must never end up aliased to a cache-owned object). Both converge on `_tl_build_and_unfreeze`. |
...
- `persistent._tl_replay_path` — list of `{index, chosen_index, ast_key}` dicts; choice history snapshot at jump time; also carries shadow path entries
- `persistent._tl_replay_target` — dict `{node_index, option_index}`; jump destination; `None` for chapter jumps
- `persistent._tl_prev_thumb` — bytes; thumbnail of the node before the jump target
- `persistent._tl_synthetic_jump` — bool; True when a snapshot is pending unfreeze on load

**Functions:**

| Function | Signature | Description |
|----------|-----------|-------------|
| `_tl_save_no_screenshot` | `(slot) → None` | Writes a save with no screenshot: uses `include_screenshot=False, mutate_flag=False` on RenPy 8+; shadows `interface.get_screenshot` with a 1×1 black PNG on RenPy 7. `mutate_flag=False` skips `location.scan()` (directory listing) after save. Used for the recovery save. |
| `_tl_pre_save_slot` | `(node_index, context, ast_key=None) → str` | Returns `_pre_{NNNN}_{hash6}` slot name; hash is MD5[:6] of `context[:node_index]` + optional `ast_key`. Used by disk fallback lookup. |
| `_tl_find_pre_save` | `(node_index, context, ast_key=None, save_dir=None) → str or None` | Checks root savedir for a pre-save matching this node+context+ast_key; returns slot name or None. Used by `_find_slot` for backward compat. |
| `_tl_save_slot` | `(node_index, context) → str` | Returns `_ch_{NNNN}_{hash6}` slot name. Used by `_find_slot` to read old checkpoint saves. |
...
| `_tl_chap_slot_exists` | `(slot_name, save_dir=None) → (bool, str or None)` | Checks both `-LT1.save` and `.save` extensions; returns `(exists, slot_name)`. |
| `_tl_clear_replay_state` | `() → None` | Clears all replay-related persistent variables and calls `renpy.save_persistent()`. |
| `_find_slot` | `(node_index, hist, context) → str or None` | Tier 1: exact `_pre_*` at target. Tier 2: walk history descending from `node_index-1`; for each node check pre-save then `_ch_*` checkpoint; also check chapter-end markers at each step. Tier 3: `_ch_start`. Returns first hit. |
| `_tl_jump` | `(node_index=None, option_index=None, chapter_label=None) → None` | Unified entry point for menu and chapter jumps. Writes recovery save, stages `persistent._tl_replay_path` (and shadow entries), looks up snapshot; if valid dispatches via `_dispatch_snap`; else falls back to `_find_slot` or chapter-end slot on disk. |
| `_tl_cancel_jump` | `() → None` | Cancels an in-progress jump; snapshots current thumbnails to `renpy.game._tl_thumb_cache`, clears replay state, triggers recovery load. |
...
| Function | Signature | Description |
|----------|-----------|-------------|
| `_tl_find_scene_seen_name` | `(start_node, max_hops=80) → str or None` | Walks the AST forward from a scene node to find the first translated say-name, used to key seen state to actual scene content. Handles Ren'Py 7 `Translate`-wrapped Say nodes. |
...
| `_tl_follow_jump_seen_name` | `(target, max_hops=30) → str or None` | Hops one level into a jump target label to find the first Say seen name; skips Python/Scene/Show prefix nodes; handles Ren'Py 7 `Translate`-wrapped Say nodes; returns None if no Say found before a Menu/Jump/Return. |
...
| `_tl_make_seen_fn` | `(block) → tuple` | Returns a picklable descriptor tuple for a branch block. Iterates the block list directly (not `.next` links). Priority: Say/TranslateSay/Translate range → Scene → expr-Show → Jump follow → `("never",)`. In Ren'Py 7 translated games, Say nodes inside menu blocks are wrapped in `Translate` AST nodes; the code dives into `node.block` to extract the Say. Plain Show nodes (all-identifier imspec, e.g. `show eileen happy`) are excluded — character sprites appear in many branches and are branch-ambiguous in `_seen_images`. Show nodes with expression args (e.g. `show txt _("кошмар")`) ARE captured as `("image", raw_parts)` because RenPy stores raw imspec parts in `_seen_images`, making them branch-specific. Multiple Says return `("say_range", first, last)`. |
...
Shadow entries use `ast_key` — the same `(filename, linenumber)` tuple as `_tl_node_menu_site_key()`. Old entries with `menu_site_key` stored as a list are coerced to tuple on lookup for backward compat.

**Functions:**

| Function | Signature | Description |
|----------|-----------|-------------|
| `_tl_shadow_match` | `(shadow_path, node) → int or None` | Returns the `chosen_index` from the first entry matching the current node by `ast_key`; returns None if no match. |
| `_tl_consume_shadow_path` | `(shadow_path, node, chosen_index) → (list or None, int or None, str or None)` | Consumes shadow path entries up to and including the first `ast_key` match; returns `(remaining_path_or_none, diverged_orig_ci_or_none, match_mode_or_none)`; `diverged_orig_ci` is set only when the player's choice differs from the original. |
...
| `_tl_on_load` | `() → None` | Registered `after_load_callback`; if `_tl_synthetic_jump` is set, hides all `config.overlay_screens` (clears `restarting=True` stale ScreenDisplayables left by `before_restart()` during unfreeze) then calls `_tl_unfreeze_from_snapshot` (never returns); otherwise clears stale replay state or resumes valid replay; reconstructs `store._tl_shadow_path` from `persistent._tl_replay_path`; migrates img_names. |
...
| `_tl_chapter_label_cb` | `(label_name, abnormal) → None` | Registered `label_callback`; fires when any chapter end label is reached; records `{chapter_name, end_label, after_index}` marker; captures and caches a chapter snapshot via `_tl_capture_snapshot` / `_tl_cache_chapter_snapshot`. |
...
| `tl_chapter_divider(chapter_name, end_label)` | `(str, str)` | Centered `—— End of {chapter} ——` divider between chapter sections; clicking calls `Function(_tl_jump, chapter_label=end_label)` then jumps to `_tl_do_chap_end_jump`. |
...
**What it tests:** Save slot naming, pre-save slot naming, pre-save file lookup, `_find_slot` tier logic, and history validation.
