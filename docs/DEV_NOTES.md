...
Ghost card synthesis. Monkey-patches `renpy.ast.If.execute` to track branch conditions at runtime.
...
| `_tl_prettify_condition` | `(cond) → str` | Replaces bare snake_case identifiers with prettified names from `_tl_prettify_var`. |
| `_tl_get_taken_branch` | `(if_node) → int` | Evaluates conditions in order and returns the index of the first True one. |
| `_tl_build_ghost_payload` | `(if_node, taken_index, context_img=None) → dict` | Builds one ghost payload dict for a single If node with conditions, seen_fns, branch_imgs. |
...
| `_tl_on_if_execute` | `(if_node, taken_index) → None` | Callback after If.execute; orchestrates ghost synthesis and visited-node marking. |
| `_tl_should_track_if_node` | `(if_node) → bool` | Returns True if the If node is from a game script (not timeline internals). |
| `_tl_if_execute_patched` | `(self) → None` | Replacement for `renpy.ast.If.execute`; evaluates taken branch before executing and calls `_tl_on_if_execute`. |
...
| `_tl_make_seen_fn` | `(block) → tuple` | Returns a picklable descriptor: `("never",)`, `("say", name)`, or `("label", target)`; handles plain-say branch starts that `_tl_make_scene_seen_fn` misses. Use this for ghost cards. |
...
| `_tl_eval_seen_fn` | `(seen_fn) → bool` | Evaluates a descriptor tuple against live RenPy state (`persistent._seen_ever`, `renpy.seen_label`). |
...
**Constants:**
- `config.keymap["chronology_toggle"] = ["t"]` — T to open/close timeline

**Functions:**

| Function | Signature | Description |
|----------|-----------|-------------|
| `_tl_toggle` | `() → None` | Toggles the timeline screen and blur overlay on/off. |
...
|--------|-----------|-------------|
| `timeline()` | none | Root screen; blur layer + dark overlay + header + scrollable card grid; builds `_tl_items` interleaving `("divider", chapter_name, end_label)` tuples from `chapter_end` node flags and `_tl_chapter_markers`; delegates card/ghost rendering to `ui/`. |
...
| `_tl_keylistener()` | none | Overlay screen; maps keybinds for `chronology_toggle` and `tl_debug_toggle`. |
...
| `tl_ghost_rows(ghost_nodes, ghost_highlight, card_w, cols, spacing)` | `(list, tuple or None, int, int, int)` | Renders all ghost nodes as clustered rows; flattens branches into a flat list, chunks into rows; inserts separator frames between clusters. |
...
**Key components:**
- **RenPy stub**: mock `renpy`, `store`, `persistent` modules with minimal AST node classes
- **`.rpy` loader**: `load_rpy(rel_path, ns)` extracts and executes all `init [priority] python:` blocks from a `.rpy` file
...
**What it tests:** New-content detection on nodes and seen-function descriptor evaluation.
