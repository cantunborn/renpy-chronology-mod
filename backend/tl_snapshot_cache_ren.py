## =============================================================================
## CHRONOLOGY MOD — tl_snapshot_cache_ren.py
...
def _tl_values_equal(a, b):  # type: (Any, Any) -> bool
    """
...
class TLSnapshotCache(object):
    ...
    def capture(self):
        ...
        False. It is then pickled to bytes (via renpy.compat.pickle, the
        same wrapper renpy/loadsave.py itself uses) rather than stored
        live: rollback_copy() shallow-copies scene_lists, aliasing
        ScreenDisplayable objects between the live context and every
        cached snapshot — pickling snapshots the whole object graph by
        value instead, closing that hole uniformly rather than auditing
        each shared sub-object individually. Since this cache rides
        inside real save files (renpy.game.log, which owns the cache, is
        pickled whole on every save — see loadsave.py), a plain stdlib
        pickle would still need to be readable after a future Ren'Py/
        Python version bump; renpy.compat.pickle already handles that.
...
def _tl_make_cache():  # type: () -> TLSnapshotCache
...
def _tl_cache_menu_snapshot(node_index, snap):  # type: (int, dict) -> None
    _tl_get_snapshot_cache().cache_menu(node_index, snap)

def _tl_cache_chapter_snapshot(label_name, snap):  # type: (str, dict) -> None
    _tl_get_snapshot_cache().cache_chapter(label_name, snap)

def _tl_get_menu_snapshot(node_index):  # type: (int) -> Optional[dict]
    return _tl_get_snapshot_cache().get_menu(node_index)

def _tl_get_chapter_snapshot(label_name):  # type: (str) -> Optional[dict]
    return _tl_get_snapshot_cache().get_chapter(label_name)

def _tl_transfer_snapshot_cache(new_log):  # type: (Any) -> None
...
def _tl_build_and_unfreeze(roots, ctx, log_prefix, rollback_limit):  # type: (dict, Any, str, int) -> None
    """
...
def _tl_unfreeze_legacy(snap):  # type: (dict) -> None
    """
...
def _tl_unfreeze_from_snapshot(snap):  # type: (dict) -> None
    """
    ...
    - {"roots": ..., "ctx": ..., "rollback_limit": ...} (current format): the
        cache-owned frozen roots are deep-copied fresh before being handed to
        Ren'Py's real unfreeze() — without this, the cache's own frozen
        ...
        shares them. ctx is a pickled bytes blob (see capture()) — unpickling
        it always produces a fresh, independent object graph, so no
        additional deepcopy is needed on this path.
