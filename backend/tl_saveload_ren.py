## =============================================================================
## CHRONOLOGY MOD — tl_saveload_ren.py
...
if TYPE_CHECKING:
    ...
    import hashlib as _tl_hashlib  # type-check-only; injected into store namespace at runtime
    from renpy import persistent, store
    from tl_menu_location_ren import _tl_node_menu_site_key  # type-check-only; injected into store namespace at runtime
    from tl_snapshot_cache_ren import _tl_get_menu_snapshot, _tl_get_chapter_snapshot  # type-check-only; injected into store namespace at runtime
    from timeline_init_ren import _tl_log, TL_THUMB_CACHE_MAX  # type-check-only; injected into store namespace at runtime
...
def _tl_save_no_screenshot(slot):  # type: (str) -> None
...
def _tl_save_slot(node_index, context):  # type: (int, list) -> str
...
def _tl_pre_save_slot(node_index, context, ast_key=None):  # type: (int, list, Optional[tuple]) -> str
    raw = repr((tuple(context[:node_index]), ast_key))
    slot_hash  = _tl_hashlib.md5(raw.encode("utf-8")).hexdigest()[:6]
    return "_pre_{:04d}_{}".format(node_index, slot_hash)

def _tl_find_pre_save(node_index, context, ast_key=None, save_dir=None):  # type: (int, list, Optional[tuple], Optional[str]) -> Optional[str]
...
def _tl_chap_end_slot_name(label, context=None, after_index=None):  # type: (str, Optional[list], Optional[int]) -> str
...
def _tl_chap_slot_exists(slot_name, save_dir=None):  # type: (str, Optional[str]) -> Tuple[bool, Optional[str]]
...
def _tl_clear_replay_state():  # type: () -> None
...
def _stage_menu_replay(hist, node_index, option_index):  # type: (list, int, int) -> None
...
def _stage_chapter_shadow(hist, after_index):  # type: (list, int) -> None
...
def _dispatch_snap(snap):  # type: (dict) -> None
    persistent._tl_synthetic_jump = True
    renpy.save_persistent()
    renpy.game._tl_pending_snap = snap

def _valid_snap(snap):  # type: (Optional[dict]) -> bool
...
def _find_slot(node_index, hist, context):  # type: (int, list, list) -> Optional[str]
    """
...
def _tl_jump(node_index=None, option_index=None, chapter_label=None):  # type: (Optional[int], Optional[int], Optional[str]) -> None
    """
...
def _tl_cancel_jump():  # type: () -> Optional[str]
    """
