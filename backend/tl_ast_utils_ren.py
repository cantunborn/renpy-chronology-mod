## =============================================================================
## CHRONOLOGY MOD — tl_ast_utils_ren.py
## Shared AST utilities used across multiple backend files.
## Loads before all other tl_*_ren.py backend files (alphabetical sort at init -2).
## =============================================================================
...
if TYPE_CHECKING:
    ...
    from timeline_init_ren import _tl_builtin_id  # type-check-only; injected into store namespace at runtime
...
_tl_pyast_Str_util = getattr(_tl_pyast_util, "Str", None)
_tl_pyast_Num_util = getattr(_tl_pyast_util, "Num", None)

def _tl_strip_renpy_tags(s):  # type: (str) -> str
    """Strip {tag} markup from s, leaving inner text."""
    return _tl_re_util.sub(r'\{[^}]*\}', '', s)

def _tl_ast_literal_value(node):  # type: (Any) -> Optional[str]
    """
    ...
    if _tl_pyast_Str_util and isinstance(node, _tl_pyast_Str_util):
        return str(node.s)
    if _tl_pyast_Num_util and isinstance(node, _tl_pyast_Num_util):
        return str(node.n)
    if isinstance(node, _tl_pyast_util.Name) and node.id in ("True", "False"):
        return node.id
    return None

def _tl_extract_compare_literals(cond_str):  # type: (str) -> List[str]
    """
...
def _tl_is_game_file(f):  # type: (str) -> bool
    """
    ...
    All mod files (including timeline_*_ren.py) live under renpy-chronology-mod/
...
def _tl_walk_ast_blocks(nodes, visitor_fn, initial_state=None):  # type: (list, Callable, Any) -> None
    """
...
def _tl_prettify_var(name):  # type: (str) -> str
