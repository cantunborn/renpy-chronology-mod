## =============================================================================
## CHRONOLOGY MOD — tl_menu_options_ren.py
...
if TYPE_CHECKING:
    ...
    from timeline_init_ren import _tl_runtime_choice_returns  # type-check-only; injected into store namespace at runtime

"""renpy
init -2 python:
"""

def _tl_parse_menu_items(items):  # type: (list) -> Tuple[str, list]
    """
...
def _tl_valid_choice_entries(items):  # type: (list) -> list
    out = []
    for entry in items:
        label = entry[0]
...
def _tl_choice_entry_for_index(items, choice_index):  # type: (list, Optional[int]) -> Optional[tuple]
...
def _tl_choice_index_from_return_value(items, rv):  # type: (list, Any) -> Optional[int]
...
def _tl_populate_choice_returns(node, items):  # type: (Any, list) -> None
    if node is None:
        return
