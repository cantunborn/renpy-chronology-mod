## =============================================================================
## CHRONOLOGY MOD — tl_seen_check_ren.py
...
if TYPE_CHECKING:
    ...
    from tl_menu_location_ren import _tl_live_menu_lookup  # type-check-only; injected into store namespace at runtime
    from tl_assets_ren import _tl_scene_stmt_img_name  # type-check-only; injected into store namespace at runtime
    from timeline_init_ren import _tl_log, TL_DEBUG_SEEN, _tl_runtime_choice_returns  # type-check-only; injected into store namespace at runtime

"""renpy
init -2 python:
"""

def _tl_find_scene_seen_name(start_node, max_hops=80):  # type: (Any, int) -> Optional[str]
...
def _tl_say_seen_name(node):  # type: (Any) -> Optional[str]
...
def _tl_follow_jump_seen_name(target, max_hops=30):  # type: (Any, int) -> Optional[str]
...
def _tl_first_scene_seen_name(block):  # type: (Optional[list]) -> Optional[str]
...
def _tl_scene_ast_id(stmt):  # type: (Any) -> Optional[tuple]
...
def _tl_make_seen_fn(block):  # type: (Optional[list]) -> tuple
...
def _tl_make_scene_seen_fn(block):  # type: (Optional[list]) -> tuple
...
def _tl_eval_seen_fn(seen_fn):  # type: (tuple) -> bool
...
def _tl_option_seen(node, option_index):  # type: (dict, int) -> bool
...
def _tl_option_peek_seen_fn(node, option_index):  # type: (dict, int) -> Optional[tuple]
    """
...
def _tl_node_has_new(node):  # type: (dict) -> bool
