...
screen tl_ghost_card(ghost, bi, cw, th, hl):
    ...
    python:
        ...
        _gbc_seen   = _gbc_taken or (
            _tl_eval_seen_fn(_gbc_sfns[bi])
            if bi < len(_gbc_sfns) else False
        )
...
screen tl_ghost_rows(ghost_nodes, ghost_highlight, card_w, cols, spacing):
    if ghost_nodes:
        ...
        python:
            ...
            _tl_gbc_flat = []
            for _gbc_gi, _gbc_g in enumerate(ghost_nodes):
                _gbc_cluster_sep = (
                    _gbc_gi > 0 and
