## =============================================================================
## CHRONOLOGY MOD — timeline_hooks_ren.py
## =============================================================================
...
if TYPE_CHECKING:
    ...
    import os  # type-check-only; injected into store namespace at runtime
    ...
    from store import _tl_history, _tl_node_count, _tl_context  # type-check-only; satisfies pyright's `global` binding check, injected into store namespace at runtime
    from timeline_init_ren import _tl_log, _tl_migrate_img_names, _tl_build_ast_map, _tl_chapters, TL_DEBUG_MENU, TL_THUMB_CACHE_MAX  # type-check-only; injected into store namespace at runtime
    from tl_menu_location_ren import _tl_menu_site_key, _tl_location_menu_site_key, _tl_node_menu_site_key  # type-check-only; injected into store namespace at runtime
    from tl_assets_ren import _tl_resolve_live_menu_img_name, _tl_img_name_is_movie, _tl_capture_thumbnail  # type-check-only; injected into store namespace at runtime
    from tl_route_logic_ren import _tl_flush_var_changes, _tl_flush_menu_snap, _tl_snapshot_route_vars  # type-check-only; injected into store namespace at runtime
    from tl_menu_options_ren import _tl_parse_menu_items, _tl_choice_entry_for_index, _tl_choice_index_from_return_value, _tl_valid_choice_entries, _tl_populate_choice_returns  # type-check-only; injected into store namespace at runtime
    from tl_snapshot_cache_ren import _tl_capture_snapshot, _tl_cache_menu_snapshot, _tl_cache_chapter_snapshot, _tl_init_snapshot_cache  # type-check-only; injected into store namespace at runtime
    from tl_shadow_path_ren import _tl_consume_shadow_path  # type-check-only; injected into store namespace at runtime
    from tl_saveload_ren import _tl_clear_replay_state, _tl_save_no_screenshot  # type-check-only; injected into store namespace at runtime

"""renpy
init -1 python:
"""

def _tl_resolve_node_location():  # type: () -> Tuple[Optional[tuple], Optional[str], Optional[str]]
    """
...
def _tl_resolve_img_name(ast_key, location):  # type: (Optional[tuple], Optional[str]) -> Optional[str]
    """
...
def _tl_capture_thumb(node, ast_key):  # type: (dict, Optional[tuple]) -> None
    """
...
def _tl_record_before(items):  # type: (list) -> Optional[dict]
...
def _tl_record_after(node, chosen_label=None, chosen_index=None):  # type: (Optional[dict], Optional[str], Optional[int]) -> None
    global _tl_context
...
if not getattr(renpy.exports.menu, "_tl_wrapped", False):
    ...
    setattr(_tl_exports_wrapper, "_tl_wrapped", True)
    renpy.exports.menu = _tl_exports_wrapper

def _tl_replay_pick(n_index, items, path, target):  # type: (int, list, Optional[list], Optional[dict]) -> Optional[tuple]
    """
...
if not getattr(renpy.store.menu, "_tl_wrapped", False):
    ...
    setattr(_tl_store_wrapper, "_tl_wrapped", True)
...
def _tl_on_game_start():  # type: () -> None
    try:
...
def _tl_interact_callback():  # type: () -> None
    if not hasattr(store, "_tl_history"):
